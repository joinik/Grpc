...
func dialTimeout(f newClientFunc, network, address string, opts ...*service.Option) (client *Client, err error) {
	...
	// close the connectionif client is nil
	defer func() {
		if err != nil {
			_ = conn.Close()
		}
...
// XDial calls different funcitons to connect to a RPC server
// according the first paramenter rpcAddr.
// rpcAddr is a general format (protocol@addr) to represent a rpc server
// e.g. http@10.0.0.1:7008, tcp@10.0.0.1:8989, unix@/tmp/Grpc.sock
func XDial(rpcAddr string, opts ...*service.Option) (*Client, error) {
	parts := strings.Split(rpcAddr, "@")
	if len(parts) != 2 {
		return nil, fmt.Errorf("rpc client err: wrong format '%s', expect protocl@addr", rpcAddr)
	}
	protocol, addr := parts[0], parts[1]
	switch protocol {
	case "http":
		return DialHTTP("tpc", addr, opts...)
	default:
